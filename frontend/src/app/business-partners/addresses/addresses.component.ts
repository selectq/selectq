...
@Component({
  ...
  template: `
    ...
    <p>Saving an edited address or GSTIN creates a new version and archives the previous version. Existing invoices retain their address.</p>
    ...
    <div class="address" *ngFor="let address of addresses; let i = index">
      ...
      <label [for]="'bp-gstin-' + i">GSTIN for this address (optional)</label>
      <input [id]="'bp-gstin-' + i" type="text" maxlength="15" [ngModel]="address.gstin || ''" [ngModelOptions]="{standalone: true}" (ngModelChange)="change(i, {gstin: $event.toUpperCase()})" [disabled]="disabled || !!address.is_archived" placeholder="e.g. 29AABCU9603R1ZP" />
      <small *ngIf="address.gstin && !validGSTIN(address.gstin)" role="alert">Enter a 15-character GSTIN beginning with a two-digit state code.</small>
  ...
  styles: [`:host {display:block;margin:1rem 0} .heading{display:flex;justify-content:space-between;align-items:center;gap:1rem} p,small{color:var(--text-secondary);font-size:.85rem;margin:.6rem 0;display:block} .address{border:1px solid var(--surface-border);padding:1rem;border-radius:8px;margin:.8rem 0} textarea,input[type=text]{display:block;width:100%;margin:.5rem 0;padding:.6rem;background:var(--input-bg);color:var(--text-primary);border:1px solid var(--surface-border);border-radius:6px;font:inherit} button{padding:.45rem .7rem;border:1px solid var(--surface-border);border-radius:6px;background:var(--input-bg);color:var(--text-primary);cursor:pointer} input[type=checkbox]{width:auto} label{display:block} textarea:disabled{opacity:.7}`]
})
export class PartnerAddressesComponent {
  ...
  validGSTIN(value: string) { return /^[0-9]{2}[A-Z0-9]{13}$/.test(value.trim().toUpperCase()); }
